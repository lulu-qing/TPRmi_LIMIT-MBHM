...
class FSCILTrainer(Trainer):
    ...
    def get_base_dataloader_meta(self):
        ...
        if self.args.dataset == 'mbhm':
            # 新增：直接从 txt 中读取真实的跳跃类别 (例如 0, 2, 5, 8)
        ...
        train_gfsl_loader = DataLoader(dataset=trainset, batch_size=self.args.batch_size_base, shuffle=True,
                                       num_workers=8, pin_memory=True)
        train_sampler = CategoriesSampler(trainset.targets, len(train_gfsl_loader), self.args.sample_class,
                                          self.args.sample_shot)
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:
                ...
                for epoch in range(args.epochs_base):
                    ...
                    self.model.eval()
                    tl, ta = self.base_train(self.model, train_fsl_loader, train_gfsl_loader, optimizer, scheduler,
                                             epoch, args)
